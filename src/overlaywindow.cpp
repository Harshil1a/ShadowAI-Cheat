...
static inline QString panicKeyPrefix() { return "Ctrl+Shift+"; }
#endif

// ─── Ghost Writer Worker Thread ──────────────────────────────────────────────
class GhostWriterWorker : public QThread {
public:
  GhostWriterWorker(const QString &text, int minDelay, int maxDelay,
                    QObject *parent = nullptr)
      : QThread(parent), m_text(text), m_minDelay(minDelay),
        m_maxDelay(maxDelay), m_stop(false) {}
  ...
  void run() override {
    ...
    for (int i = 0; i < m_text.length(); ++i) {
      ...
      if (qc == '\n') {
        ...
        // Type a dummy space to guarantee the line is never empty (prevents
        // Backspace from deleting the newline)
        INPUT spacePress[2] = {};
        ...
        // Send Shift+Home (press Home twice to guarantee selecting to absolute
        // start of line in Monaco/VS Code)
        // 1. Shift down
        INPUT shiftDown = {};
        ...
        // 3. Second Home press (forces cursor to column 0 if first stopped at
        // non-whitespace)
        SendInput(2, homePress, sizeof(INPUT));
      ...
      bool isAutoCloseChar =
          (qc == '{' || qc == '(' || qc == '[' || qc == '"' || qc == '\'');
      if (isAutoCloseChar) {
        // Wait 30ms for the editor to auto-insert the closing character
        QThread::msleep(30);

        // Send simulated Delete keypress to wipe out the auto-inserted
        // character
        INPUT deletePress[2] = {};
...
OverlayWindow::OverlayWindow(QWidget *parent)
    : QWidget(parent), m_loadingTimer(new QTimer(this)) {
  setWindowFlags_();
  setupUI();
  applyGlassStyle();

  m_audioRecorder = new AudioRecorder(this);
  connect(m_audioRecorder, &AudioRecorder::recordingFinished, this,
          &OverlayWindow::onRecordingFinished);
  connect(m_audioRecorder, &AudioRecorder::errorOccurred, this,
...
void OverlayWindow::setupUI() {
  ...
  connect(&AccountManager::instance(), &AccountManager::accountStateChanged,
          this, [this](bool, const QString &, bool) { updateCreditsBadge(); });
  updateCreditsBadge();
  ...
  QLabel *placeholder = new QLabel(
      QString("📷  No screenshots yet — press %1S to capture").arg(modKeyPrefix()));
...
void OverlayWindow::setAIManager(AIManager *ai) {
  ...
  connect(ai, &AIManager::transcriptionOnlyFinished, this,
          &OverlayWindow::onTranscriptionOnlyFinished);
}

void OverlayWindow::setScreenCapture(ScreenCapture *sc) { m_sc = sc; }
...
void OverlayWindow::clearAll() {
  ...
  QLabel *placeholder =
      new QLabel(QString("Cleared. Press %1S to capture again.").arg(modKeyPrefix()));
  placeholder->setObjectName("screenshotThumb");
  placeholder->setAlignment(Qt::AlignCenter);
...
static QString vkToKeyName(int vk) {
  ...
  case 0x28:
    return "Down";
  default:
    return QString("0x%1").arg(vk, 2, 16, QChar('0')).toUpper();
  }
}

void OverlayWindow::refreshKeyBadges() {
  if (!m_keysLayout1 || !m_keysLayout2)
    return;
  ...
  auto makeKey = [](const QString &key, const QString &action,
    ...
    if (isPanic) {
      kLbl->setStyleSheet(
          "background: rgba(255, 60, 60, 0.25); border: 1px solid #ff4444; "
          "color: #ff6b6b; font-weight: bold; border-radius: 4px; padding: 2px "
          "6px; font-size: 10px; font-family: 'Consolas', monospace;");
    }
...
void OverlayWindow::onAIError(const QString &err) {
  ...
  if (found && nextSlot != currentSlot) {
    ...
    m_answerDisplay->setPlainText(
        ...
        QString(".\nPress Get Answer (%1A) to retry.").arg(modKeyPrefix()));
  } else {
...
QString OverlayWindow::markdownToHtml(const QString &text) {
  ...
  for (const QString &rawLine : lines) {
    ...
    if (trimmed.startsWith("```") || trimmed.startsWith("~~~")) {
      if (!inCodeBlock) {
        ...
        html +=
            QString(
                "<pre style=\"background:rgba(0,0,0,0.4); padding:8px 10px; "
    ...
    if (trimmed.startsWith("#### ")) {
      html +=
          QString("<p style=\"color:#cbd5e1; font-size:13px; font-weight:bold; "
                  "margin:8px 0 3px;\">%1</p>")
              .arg(processInline(trimmed.mid(5)));
    } else if (trimmed.startsWith("### ")) {
      html +=
          QString("<p style=\"color:#e2e8f0; font-size:14px; font-weight:bold; "
                  "margin:9px 0 3px;\">%1</p>")
              .arg(processInline(trimmed.mid(4)));
    } else if (trimmed.startsWith("## ")) {
      html +=
          QString("<p style=\"color:#f1f5f9; font-size:15px; font-weight:bold; "
                  "margin:10px 0 4px;\">%1</p>")
              .arg(processInline(trimmed.mid(3)));
    } else if (trimmed.startsWith("# ")) {
      html +=
          QString("<p style=\"color:#ffffff; font-size:16px; font-weight:bold; "
                  "margin:12px 0 5px;\">%1</p>")
              .arg(processInline(trimmed.mid(2)));
    }
    // ── Blockquote ──
    else if (trimmed.startsWith("> ")) {
      html +=
          QString("<div style=\"border-left:3px solid rgba(100,160,255,0.5); "
    ...
    // ── Regular paragraph ──
      ...
      html += QString("<p style=\"margin:3px 0; color:#e2e8f0; "
                      "line-height:1.5;\">%1</p>")
  ...
  if (inCodeBlock && !codeContent.isEmpty()) {
    html +=
        QString("<pre style=\"background:rgba(0,0,0,0.4); padding:8px 10px; "
                "border-radius:6px; font-family:Consolas,monospace; "
                "font-size:12px; "
                "color:#e2e8f0; margin:6px 0; white-space:pre-wrap;\">%1</pre>")
            .arg(codeContent.toHtmlEscaped());
...
void OverlayWindow::updateScreenshotThumb(const QPixmap &px) {
  ...
  thumb->setStyleSheet("border: 1px solid rgba(0, 229, 255, 0.35); "
                       "border-radius: 6px; background: #0d121f;");

  QPixmap scaled =
      px.scaled(thumb->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation);
...
void OverlayWindow::showStatusMessage(const QString &msg, bool isError) {
  ...
  if (isError) {
    ...
    m_statusLabel->setStyleSheet("color: rgba(0, 229, 255, 0.75); font-size: "
                                 "11px; font-family: Consolas;");
  }
...
void OverlayWindow::toggleVoiceRecord() {
  if (m_isRecordingAudio) {
    ...
    QString tempPath = QDir::tempPath() + "/sys_audio_" +
    ...
    if (!m_transcribeTimer) {
      ...
      connect(m_transcribeTimer, &QTimer::timeout, this,
              &OverlayWindow::onTranscribeTimerTimeout);
    }
...
void OverlayWindow::stopVoiceRecordOnRelease() {
  // If we are recording and have held the key for longer than 400ms, stop
  // recording. Otherwise, treat it as a brief tap (to toggle).
  if (m_isRecordingAudio && m_recordTimer.isValid() &&
      m_recordTimer.elapsed() > 400) {
    m_isRecordingAudio = false;
    if (m_transcribeTimer) {
      m_transcribeTimer->stop();
    }
...
void OverlayWindow::onTranscribeTimerTimeout() {
  if (!m_isRecordingAudio || !m_audioRecorder)
    return;
...
void OverlayWindow::onRecordingFinished(const QString &filePath) {
  ...
  if (provider == "gemini") {
    ...
    if (m_ai->isBusy()) {
      m_answerDisplay->setPlainText(
          "⚠️ AI is busy. Please wait for the current query to complete.");
    ...
    m_ai->askWithImages(m_screenshots,
                        "Solve / answer the question using the provided audio "
                        "and screenshot(s).",
                        base64Audio, "audio/wav");
    ...
    m_answerDisplay->setPlainText(
        QString("❌ Audio recording is not supported for provider: %1")
            .arg(provider));
...
void OverlayWindow::doGhostWriter() {
  ...
  m_ghostWriterWorker = new GhostWriterWorker(
      textToType, cfg.ghostWriterMinDelay(), cfg.ghostWriterMaxDelay(), this);

  connect(m_ghostWriterWorker, &QThread::finished, this,
          [this]() { showStatusMessage("Typing completed ✓"); });

  m_ghostWriterWorker->start();
}

QString OverlayWindow::extractCodeBlock(const QString &fullText,
                                        bool smartIndent) {
...
void OverlayWindow::updateCreditsBadge() {
  ...
  if (isPro) {
    ...
    m_creditsBadge->setStyleSheet(
        "color: #00ff66; font-size: 10px; font-weight: 800; font-family: "
        "'Consolas', monospace; background: rgba(0, 255, 102, 0.15); border: "
        "1px solid rgba(0, 255, 102, 0.4); border-radius: 4px; padding: 1px "
        "6px;");
    ...
    if (credits > 0) {
      ...
      m_creditsBadge->setStyleSheet(
          "color: #00e5ff; font-size: 10px; font-weight: 800; font-family: "
          "'Consolas', monospace; background: rgba(0, 229, 255, 0.15); border: "
          "1px solid rgba(0, 229, 255, 0.4); border-radius: 4px; padding: 1px "
          "6px;");
    } else {
      m_creditsBadge->setText(QString::fromUtf8("🪙 0 Solves"));
      m_creditsBadge->setStyleSheet(
          "color: #ff4757; font-size: 10px; font-weight: 800; font-family: "
          "'Consolas', monospace; background: rgba(255, 71, 87, 0.15); border: "
          "1px solid rgba(255, 71, 87, 0.4); border-radius: 4px; padding: 1px "
          "6px;");
    }
